...
def do_deploy(archive_path):
    ...
    try:
        ...
        run('tar -xzf /tmp/{} -C {}{}/'.format(file_name, path, file_name[:-4]))
        run("rm /tmp/{}".format(file_name))
        run("mv {}{}/web_static/* {}{}/".format(path, file_name[:-4], path, file_name[:-4]))
        run("rm -rf {}{}/web_static".format(path, file_name[:-4]))
        run("rm -rf /data/web_static/current")
        run("ln -s {}{}/ /data/web_static/current".format(path, file_name[:-4]))
        return True
    except:
        return False
